...
def skin_detect(image_in: str, image_out: str):
  ...
  alpha = np.true_divide(D1Cb, D1Cr + epsilon)
  epsilon = 1e-10
  dCbS = np.zeros_like(alpha + epsilon)
  ...
  #  Condition C.1
  epsilon = 1e-10  # A small number to prevent division by zero
  Jvals = np.multiply(dCbS, np.true_divide((dCbS + dCr), (D1Cb + D1Cr + epsilon)))
...
#   image_in = "/data/test_face_photos/DSC01418.JPG"
#   image_out = "skinMask.png"
#   skin_detect(image_in, image_out)


    # Input and output directories
    input_directory = "../data/test_face_photos"
    output_directory = "../important_outputs/skins"
    ...
    for filename in os.listdir(input_directory):
        if filename.endswith(".JPG") or filename.endswith(".jpg"):
            ...
            output_image_path = os.path.join(output_directory, f"skin_{filename}")

            # Apply skin detection to the current image and save the result
            skin_detect(input_image_path, output_image_path)
